 ...
 * Java汉字转换为拼音
 */
public class CharacterParserFactory {

    private final static int[] PY_VALUE = new int[]{-20319, -20317, -20304, -20295,
    ...
    private final static String[] PY_STR = new String[]{"a", "ai", "an", "ang", "ao",
    ...
    private static CharacterParserFactory instance;

    public static CharacterParserFactory getInstance() {
        if (instance == null) {
            synchronized (CharacterParserFactory.class) {
                if (instance == null) {
                    instance = new CharacterParserFactory();
                }
            }
        }
        return instance;
    ...
    private int getChsAscii(String chs) {
        try {
            ...
            int count = TextUtils.count(bytes);
            if (count == 1) {
                return bytes[0];
            } else if (count == 2) {
    ...
    public String getSelling(String chs) {
        ...
        for (int i = 0; i < chs.length(); i++) {
            ...
            if (TextUtils.count(key.getBytes()) >= 2) {
